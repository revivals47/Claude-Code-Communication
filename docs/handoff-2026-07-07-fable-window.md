...
## §F. home-PC 引き継ぎ (PRESIDENT 最終監査 追補、work-PC session 末)

### F-1. home-PC 再開手順
1. **3 repo pull**: GUI_kit → `0e5e2b0` / hayate-kit-testruct → `ffe308b` / Claude-Code-Communication → userfork 最新 (本追補 commit)。testruct-v3 も **fetch 必須** → `ee5f49a` (Win 1.0.9 取込済。監査/dispatch 前 fetch 規範)。
2. **golden**: home-PC では golden の一部 fail が出るのが正常 (fontconfig env-drift、work-PC が canonical)。**home-PC で GOLDEN_BLESS 実行禁止**。非 golden fail のみ regression signal ([[feedback_golden_env_drift]])。
3. **AppImage**: 実体 (27.5MB) は work-PC local (`~/Documents/hayate-kit-testruct-track3/target/appimage/`) で **git に無い**。home-PC で必要なら `./scripts/build-appimage.sh` で再生成 (appimagetool の導入が必要、work-PC は ~/.local/bin 導入済)。user live-verify Part B (B-2 節 4 項 + screenshot) は work-PC 持ち越しでも可。
4. PRESIDENT/boss1/worker は fresh spawn (context = 本 doc §A-§F + memory)。
5. work-PC local の古い stash 5 件 (GUI_kit、5 月期の stray-* 等) と squash-merge 済み旧 local branch 群は本 session 由来ではなく無害 (帰属ラベル付き温存)。

### F-2. ★memory mirror (work-PC → home-PC、本 session の memory 変更)

**(a) `project_testruct_public_release.md` へ 2026-07-07 進捗 2 段落を追記** (home-PC は 2026-07-04 起案の canonical を保有 → 以下 verbatim を「進捗」節へ追加 + 「残」を差し替え):

```markdown
- **2026-07-07 (work-PC、Fable window で Linux 側公開 blocker 全消化、25 PR)**: ①複数行テキスト編集 (Mac 同型中央 modal、Enter=改行/Ctrl+Enter=確定、kit #323 + testruct #88) ②描画 fidelity 完遂 = K4 グラデ / K8 SaveLayer 真 alpha + K4b polygon grad 吸収 / K9 text alpha + 影 Gaussian blur + 全 node opacity ③R-1 AppImage 配布 (27MB 実証、user 裁定) ④R-2 README 教員向け刷新 ⑤P0-2a format_version + Fill::Unknown = Linux 未来ファイル完全耐性 ⑥P0-2e cross-impl round-trip harness (scripts/cross-impl-roundtrip.sh、Rust segment 10 PASS、Swift/C# は SKIP 縮退) ⑦UI 束 (未保存確認/白紙 A4 起動/About)。
- **2026-07-07 深夜 live-verify**: Part A 全 PASS + finding 4 件を即応 root-fix で全 close (①Inspector 文字揃え不反映 = kit #328 RichTextAlign / ②縦書きトグル不在 = #96 / ③縦書き ー/〜 横向き = 回転文字クラス kit #329 + PDF #98、vert feature parity は follow-up / ④③の PDF 側配置ズレ = kit #330 baseline pivot 補正、真因 = kit PDF baseline 式の回転非考慮)。user 最終判定『PDF 出力は完璧、Mac と遜色ない』+ finding 4 再確認『バッチリ』。教訓 = PDF gate は content op assert だけでなく pdftoppm raster 並置 view 必須 (fixture に対象字形が無い偽陰性にも注意、vertical_glyph_rotation.testruct を corpus 常設)。window 累計 25 PR (GUI_kit #323-330 / testruct #85-98)、最終 HEAD = GUI_kit 0e5e2b0 / testruct ffe308b。
- **残 (更新)**: user@Mac 枠 = P0-2b/c/d (設計 = testruct docs/design/p0-2-format-compat-ci.md) + LP-4。Linux 側 = live-verify 残 (A-6 gamma 見比べ / Part B AppImage GUI 4 項 / README screenshot 3 枠)。zoom 絶対モデル化は doc 完結・実装 trigger 待ち。
```

**(b) 注記: work-PC は 2026-07-03〜05 home-PC 起案 memory 5 件を handoff doc から再構成した** (project_testruct_public_release / reference_guikit_gamma_pipeline / reference_guikit_sdf_large_text_blur / feedback_hayate_vs_gtk_live_compare / reference_wayland_stuck_grab_vt_switch)。当時の handoff に verbatim 本文が無く名前+要約のみだったため。**字面差があれば home-PC canonical が正**。今後は起案側 handoff に full body verbatim embed を徹底 (mirror 規範の再確認)。
