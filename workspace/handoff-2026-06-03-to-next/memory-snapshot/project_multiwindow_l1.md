...
description: GUI_kit multi-window L1 phased migration (S0-S5) — S0-S4+S5a+S5b-1 land 完了 (main 236ce0c)、次=S5b-2 (open_window_live で実 2 窓目 + 初の user-live AC1)
metadata: 
  node_type: memory
  type: project
...
- **S5 (最終 stage、機能本体) — sub-phasing LOCK-IN 済** (PRESIDENT 2026-06-02、SD 同格): S5a→S5b→{S5c,S5d sequential}、plumbing-first 哲学 (bit-exact refactor 先行 + 機能 flip=2窓目 open を S5b に集中 + AI-verifiable headless 最大化、user-live を S5b+ に寄せる)。infra (Window per-window field / 33 Dispatch impl / surface_to_window / windows HashMap / open_window) は S1-S4 で構築済・single-entry 維持、S5 = 実 multi-entry 化
  ...
  - **S5b** ★S5b-1 ✅ MERGED 2026-06-03 (PR #221 → main 236ce0c、squash 単一 stage commit、c1-c6 + design docs、platform lib 843/0・golden 12-2 env-drift bit-exact・examples 緑・dogfood 8 cargo check・非test warning-clean、全 1-entry bit-exact headless、codex impl-review APPROVE-WITH-NITS=code defect ゼロ nit=doc-drift c6 解消)★ — design draft v0.7 (`GUI_kit-s5b/workspace/worker1-notes/s5b_design_draft.md`、706 行) が codex design-gate 6-round で単調収束 (REVISE finding 7→3→3→1→1→0-structural → APPROVE-WITH-NITS、nit=documentary のみ)。boss1 authoritative + PRESIDENT 独立 verify 各 round。**核心設計**: (1) runtime Window build = ★option(b) factory-closure★ (App 生存中に Rc capture する make_window_factory closure を Connection へ register、decorations は非Clone+mem::take ゆえ per-call 第3引数、toplevel primary も factory 経由統一/layer-shell legacy) (2) ★channel split 3-category★ = 所有粒度 ≠ 配送 routing 分離: pure per-window (window_action/move/title/size) / focus-routed (paste→keyboard_focus_window / IME→ime_focus_window、★Enter-time owner-resolve+retain=Case A、popup focus 時も owner WindowId を retain、drain 直読、window_id_from_popup_surface 新 helper) / pure seat-global (clipboard/cursor/dnd=connection-global 1 read) (3) outbound drain + ★inbound event_fn★ 両方 windows iterate 一般化 (4) WindowState lifecycle 再導入 (Window.lifecycle、4状態 Pending→Active→CloseRequested→Closed、★9 assertion A1-A9、no-consumer=bit-exact A9 + primary residency A8) (5) open_window_live atomic API (insert→create_window_surface)。**★3 sub-split**: S5b-1 (bit-exact tier: factory+channel split+outbound/inbound drain 一般化+focus owner-resolve、全 1-entry bit-exact、headless 完結) → S5b-2 (open_window_live で実 2 窓目+id allocator+GPU/vk runtime、★初の user-live AC1) → S5b-3 (lifecycle+9test+per-window close=quit_flag decouple+title hardcode 廃止)。**PRESIDENT 2 連続 miss を多層体制が catch**: m6 (popup-focus 時 focus=None が 1 窓でも bit-exact 崩す、4回目 gate) + v0.6 owner-resolve drain 時実装不能 (5回目 gate) — 両とも「1-window bit-exact」に集中し「2-window で必要 state が使用時点に手元にあるか=実装可能性」を未検証だった ([[feedback_verify_fn_scope_by_lexical_range]] 系)。**gating-cap (PRESIDENT 5 条件、無限 gate 回避)**: 4回目 gate clean/nits→impl GO だが ①残 design-determining RESOLVE 必須 ②honest 分類 (real を nit 偽装禁止) ③pre-cargo PoC 必須 ④bit-exact gate 5 基準 ⑤micro 明示 list 化。**impl gate**: §9.2 PoC 4 領域 (factory associated-fn / outbound iterate / inbound iterate / Enter-time owner-resolve) cargo 緑 → boss1 APPROVE → full impl → bit-exact gate (cargo -j1/golden env-drift/dogfood 8/非test warning-clean/examples) → codex impl-review → PRESIDENT verify。★S5b-1 LANDED (PoC 4 領域 cargo 緑 → c1-c6 各 boss1 独立 cargo で bit-exact 確認 [lib 842/0→843/0 + golden 12-2 不変] → codex impl-review defect ゼロ → squash merge #221)★。**★impl-plan grep audit が設計 6 gate + boss1 + PRESIDENT 全員見落とした bit-exact finding catch**: §3 per-window channel は全て public App handle (`App::window_action()` 等 = `Rc::clone(&self.field)`) を持ち build_window が同 Rc を window に焼く → 設計の「per-window new」を primary に適用すると handle 切断 = bit-exact 違反 → ★re-scope (PRESIDENT confirm): S5b-1 = factory 全 channel capture (App handle 接続維持=bit-exact)、per-window NEW-isolation は S5b-2 へ★ ([[feedback_shared_to_perinstance_handle_audit]])。次 = S5b-2
...
★現在地 (2026-06-03)★ = **S5b-1 ✅ MERGED (PR #221 → main 236ce0c、squash) → 次 = S5b-2 dispatch-routing (cat1/2/3) impl-spec + S5b-2b (cat4 popup/buffer) design 並行**。【★PRESIDENT a2-iii split 2026-06-03 (SD 同格、pre-authorized trigger 発火): dispatch-handler id-routing が codex 3-round で popup/buffer (cat4=a2-iii) cluster に深さ集中 (round3 で 3 NEW structural: create_popup primary-bound owner-API 要 / buffer UserData tag が DMA-BUF path 漏れ=dmabuf.rs create_immed () / popup_surface_to_id.remove unhookable) → S5b-2 を cat1/2/3 (a2-i RESOLVED+a2-ii clean、bit-exact、advance) と ★S5b-2b cat4 popup/buffer (split-off、専用 design cycle)★ に分割。stage 構成: S5b-2 (cat1/2/3 dispatch) → S5b-2b (cat4 popup/buffer dispatch) → S5b-3 (open_window_live 2窓、初 user-live AC1) → S5b-4 (lifecycle)。S5b-2/S5b-2b 両方 bit-exact headless ゆえ 2窓 flip 前に land。dispatch-routing 確立規範: 2-arg window! macro / 全 33 Dispatch 4-category (surface/seat-focus/scale/owned-object) no-silent-omission / graceful-missing (late-event-after-close ignore) / snapshot-then-drop (whole-state call の borrow) / Case A 4-focus (pointer/keyboard/dnd/ime owner-resolve) / leave-side snapshot-before-clear (4 site) / buffer WindowId UserData (map→scan→UserData 収束)】。↓『S5b-2 carry list』記述は S5b-3 (2窓) 内容。【★PRESIDENT stage re-phasing 2026-06-03 (SD 同格): dispatch-routing(a2) を独立 bit-exact stage 抽出 (3 round 連続 structural 深掘り=primary-only→category 不完全→PopupId/wl_buffer、S5b-1 同型 deep refactor ゆえ)。新構成: **S5b-2=dispatch-handler id-routing** (bit-exact headless 先 land、33 Dispatch impl 4-category conversion[cat1 surface=2-arg window! macro/cat2 seat-focus+pointer-dnd Case A 拡張/cat3 scale=WindowId UserData 付与/cat4 owned-object=buffer/data_source/popup]+globals 16 非対象明示+2-form macro[nearly-all-graceful=late-event ignore]+serial を pointer_focus から。NEW: PopupId per-window 採番=non-unique→owner WindowId を UserData 同梱[案A]/wl_buffer popup-pool のみ owner 要/wl_touch defer。worktree GUI_kit-s5b2、draft s5b2_design_draft.md §3.5 full 化中) / **S5b-3=open_window_live 2窓 materialization** (★初 user-live AC1、a2 prerequisite 後、capture-vs-fresh[PerWindowChannels 6→9 incl DnD request]+id+GPU-vk+dnd owning-route+popup e2e、↓『S5b-2 carry list』記述は本 S5b-3 内容) / **S5b-4=lifecycle**(9-assert+per-window close+title 廃止)】。S5b-1 = bit-exact tier 完了 (factory-closure all-capture + channel split 3-cat encode + outbound/inbound drain 一般化 + Case A focus owner-hold、全 headless 1-entry bit-exact、codex impl-review defect ゼロ)。**S5b-2 dispatch (別 session 推奨)** の起点 = design draft (`GUI_kit-s5b/workspace/worker1-notes/s5b_design_draft.md`) の S5b-2 carry list = open_window_live (実 2 窓目 materialize) + Connection id allocator (next_window_id seed=1 monotonic) + per-window GPU-vk runtime init (conn field stash §7.3) + per-window NEW-isolation (primary=App Rc capture / 2窓目=fresh cell の区別機構、★2窓目に App public handle 無し=fresh 正) + dnd source-state owning-window route (connection.rs:852-905) + popup-owner resolution end-to-end test (2-window 実 surface) + ★初の user-live AC1 (2 窓同時可視) + AC3 routing。worktree GUI_kit-s5b 保持 (S5b-2 は main rebase 後 track-multiwindow/s5b-2 推奨)。S5b-3 = lifecycle 9-assert + per-window close (quit_flag decouple) + title hardcode 廃止。

[旧] S5a ✅ MERGED 2026-06-02 03:36 (PR #220 main d50bbba)、次は **S5b dispatch** (S5 の本丸 = 初の runtime 2窓目 open + 初の user-live verify、別 session 推奨)。S5b 核心 = App 非存在の running connection 上で Window を build+wire する経路 (S4 build_window は App 内、runtime live insertion 経路が未踏)。design-gate でここを最重点 scrutinize。AC1(2窓同時可視)/AC3(routing) は user-live carve。S5a で render plumbing 全 path per-window 化済ゆえ S5b は「実際に 2 個目を生やす」に集中可。
